...
export function PosDashboard({ user }: { user: SessionUser | null }) {
  ...
  const handleQtyInputCommit = (line: SaleLineItem) => {
    if (!currentSaleId || !canWrite) return;
    const raw = qtyInputs[line.id];
    setQtyInputs((prev) => {
      const next = { ...prev };
      delete next[line.id];
      return next;
    ...
    if (isNaN(newQty) || newQty === line.quantity) return;
    if (newQty < 1) {
      removeLineMutation.mutate({ saleId: currentSaleId, lineId: line.id });
    } else {
      updateLineMutation.mutate({ saleId: currentSaleId, lineId: line.id, quantity: newQty });
                          ...
                          {canWrite && isDraft ? (
                            <div className="flex items-center gap-1 shrink-0">
                              ...
                              <input
                                type="number"
                                min="1"
                                className="w-12 text-center text-sm font-medium border rounded px-1 py-0.5 [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
                                aria-label="Item quantity"
                                value={qtyInputs[line.id] ?? String(line.quantity)}
                                disabled={
                                  updateLineMutation.isPending || removeLineMutation.isPending
                                }
                                onChange={(e) =>
                                ...
                                onBlur={() => handleQtyInputCommit(line)}
                                onKeyDown={(e) => {
